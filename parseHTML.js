...
function parseHTMLColor(color) {
  ...
  if (color[0] === '#' && color.length === 7) {
    ...
    var colorVal = "#" + color[1] + color[1] + color[2] + color[2] + color[3] + color[3];
    console.log(colorVal);
    return hexToRGB(colorVal);

  } else {
    var colorName = colours[color.toLowerCase()];
...
function hexToRGB(val) {
  ...
  var result = {'r': '',
                'g': '',
                'b': ''};
  var rInfo = val.slice(1, 3);
  var gInfo = val.slice(3, 5);
  var bInfo = val.slice(5, 7);
    // console.log(rInfo);
